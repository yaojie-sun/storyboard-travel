...
export function resolveImageDisplayUrl(imageUrl: string): string {
  // 诊断：检测 __img_ref__ 泄漏
  if (imageUrl.startsWith('__img_ref__')) {
    console.error(`[resolveImageDisplayUrl] 检测到 __img_ref__ 编码未解码! value=${imageUrl}`);
    return imageUrl; // 让上游捕获该问题
  }
  ...
  // Windows 路径反斜杠标准化：convertFileSrc 会保留 \ 为 %5C，
  // 而 Tauri v2 asset protocol 不处理 %5C 编码，导致 404。
  // 替换为 / 后 convertFileSrc 生成 http://asset.localhost/C:/path/... 格式，可正常加载。
...
export function isAssetLocalhostUrl(url: string): boolean {
  return /^https?:\/\/asset\.localhost\//.test(url);
}

function extractAssetLocalhostPath(url: string): string {
  const afterPrefix = url.replace(/^https?:\/\/asset\.localhost\//, '');
  try {
    return decodeURIComponent(afterPrefix);
  } catch {
    return afterPrefix;
  }
}
...
export async function imageUrlToDataUrl(imageUrl: string): Promise<string> {
  ...
  if (isAssetLocalhostUrl(imageUrl) && isTauri()) {
    const localPath = extractAssetLocalhostPath(imageUrl);
    try {
      return await loadImage(localPath);
    } catch (error) {
      throw createImagePipelineError('无法读取本地图片数据', `source=${imageUrl}\nlocalPath=${localPath}`, error);
    }
  }
