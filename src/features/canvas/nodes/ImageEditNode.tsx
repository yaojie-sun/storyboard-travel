...
import { bananaCheckCredits } from '@/commands/ai';
...
export const ImageEditNode = memo(({ id, data, selected, width, height }: ImageEditNodeProps) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // 检查是否为余额不足错误
      if (resolvedError.message.includes('INSUFFICIENT_CREDITS') ||
          resolvedError.message.includes('次数不足') ||
          resolvedError.message.includes('剩余次数不足')) {
        // 触发打开设置对话框的"我的"标签页事件，而不是直接显示充值对话框
