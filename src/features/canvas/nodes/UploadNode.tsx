  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // 上传完成，立即持久化项目数据（绕过所有 debounce，保护用户资产）
        try {
          const pStore = useProjectStore.getState();
          if (pStore.currentProjectId) {
            ...
            pStore.saveCurrentProject(freshNodes, freshEdges, undefined, freshHistory, { immediate: true });
          }
        } catch (persistErr) {
          console.warn('[UploadNode] immediate persist after upload failed', persistErr);
        }
