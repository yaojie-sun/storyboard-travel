...
fn prepare_node_image_from_bytes(
    ...
    let preview_exists = std::path::Path::new(&preview_image_path).exists();
    let original_exists = std::path::Path::new(&image_path).exists();

    info!(
        "prepare_node_image done [{}]: bytes={}, ext={}, size={}x{}, max_preview={}, probe={}ms, decode={}ms, persist_original={}ms, resize={}ms, total={}ms, main_exists={}, preview_exists={}, main_path=\"{}\", preview_path=\"{}\"",
        trace_tag,
        bytes.len(),
        extension,
        width,
...
pub async fn save_image_source_to_app_debug_dir(
    ...
    let target_dir = crate::sync::get_user_dir(&app)?.join("debug").join(normalized_category);
...
pub async fn load_image(app: AppHandle, file_path: String) -> Result<String, String> {
    ...
    // 尝试直接读取，同时记录 images 目录路径便于诊断
    let images_dir = crate::sync::get_user_dir(&app)?.join("images");
    info!("  images_dir: {}", images_dir.display());
    info!("  file_path exists: {}", std::path::Path::new(&file_path).exists());

    let image_data = std::fs::read(&file_path)
        .or_else(|e1| {
            // 尝试从 images 目录按文件名读取
            if let Some(filename) = std::path::Path::new(&file_path).file_name() {
                let reconstructed = images_dir.join(filename);
                if reconstructed.to_string_lossy() != file_path {
                    info!("  retry from reconstructed: {}", reconstructed.display());
                    if reconstructed.exists() {
                        return std::fs::read(&reconstructed)
                            .map_err(|_| e1);
                    }
                }
            }
            // 最后尝试正斜杠/反斜杠替换
            let alt = file_path.replace('/', "\\");
            let alt2 = file_path.replace('\\', "/");
            let try_alt = if alt != file_path { Some(alt) } else { None };
            let try_alt2 = if alt2 != file_path { Some(alt2) } else { None };
            if let Some(ref p) = try_alt {
                info!("  retry backslash version: {}", p);
                if let Ok(data) = std::fs::read(p) { return Ok(data); }
            }
            if let Some(ref p) = try_alt2 {
                info!("  retry forward-slash version: {}", p);
                if let Ok(data) = std::fs::read(p) { return Ok(data); }
            }
            Err(e1)
        })
