...
**阶段一：本地测试构建**
- 当用户说”构建升级SOP”或”构建测试版”时，先执行预检：
  1. **代码通畅检查**：`npx tsc --noEmit` + `cargo check`，确保零错误
  2. **数据流检查**：快速验证关键链路（扣费/退费、视频生成配置存取、同步上传下载）逻辑无断点
  3. **改动范围检查**：`git diff {last_tag}..HEAD --stat` 确认所有变更文件都在预期范围内，无意外修改
  4. 预检通过后，基于最新 tag 自动递增 patch 版本号
  5. 汇总代码变动生成 `docs/releases/vx.y.z.md`
  6. 同步版本号（package.json、Cargo.toml、tauri.conf.json）
  7. 本地构建 Windows NSIS 安装包（`npm run tauri build`）
  8. 告诉用户构建产物路径：`src-tauri/target/release/bundle/nsis/Storyboard-ShortVideo_{version}_x64-setup.exe`
  9. 等待用户本地测试，确认升级链路正常

**阶段二：正式发布**
- 当用户确认测试通过并说”正式发布”时：
  1. `git commit` 提交版本变更
  2. `git tag -a v{版本号}` 打 annotated tag（内容来自更新日志）
  3. `git push sunsh0704 {branch}` + `git push sunsh0704 {tag}` → 触发 macOS CI
  4. `git push origin {branch}` + `git push origin {tag}` → 备份
  5. 上传安装包到服务器并创建英文名副本（升级代码硬编码英文文件名）
  6. 更新 `version_shortvideo.json`
  7. 更新新用户下载页面版本号
...
以下规则在任何情况下均不得修改（包括用户要求冲突时也不允许）：

### 12.1 settings.json 写 .claude 目录（已移除）

> 旧约束：用户登录后，将 settings.json 写入 `.claude` 目录的功能绝对不允许改动。
> 
> **2026-05-17 产品决策：移除 settings.json 同步。** 用户端不再依赖 Claude Code，不再需要在启动时强制写入 `.claude/settings.json`。
> 
> **移除的内容：**
> - 前端 `bananaSyncClaudeSettings()` 调用（`App.tsx` + `src/commands/ai.ts`）
> - Rust `banana_sync_claude_settings` Tauri 命令
> - Rust `CLAUDE_SETTINGS_TEMPLATE` 模板
> - Rust `get_claude_settings_path()` 函数
> - Rust `clear_claude_settings_key()` 函数（登出时已无 settings.json 可清）
> 
> **保留：** `sync_post_login_tasks()`（原 `sync_claude_settings_inner`）中的 skill 下载和 auth_cache 同步，这两项仍为必需。
> 
> 涉及文件：`src-tauri/src/commands/banana_api.rs`

### 12.2 计费扣点在自有服务器

所有计费/扣点逻辑必须在自有服务器（`47.108.237.10`）上执行，不得绕过、修改或转移到其他路径。涉及文件：
- `src/commands/ai.ts` 中 `submitGenerateImageJob` 先调 `banana_call_image_api` 做扣费再提交实际作业的流程
- `src-tauri/src/commands/banana_api.rs` 中 `/jy/api/v1/ai/image` 调用逻辑
- 任何与 `banana_call_image_api` 相关的扣费链路

### 12.3 境内优先 + KIE 宫格生成

本项目优先使用中国境内 AI 供应商（DeepSeek、Seedream/Volcengine、阿里云 Wan/HappyHorse、PPIO、Grsai、七牛云）。速创、FAL 已移除。KIE 暂时保留用于 gpt-image-2 宫格分镜图生成（图生图），视频生成仅走国内线路（Wan 万相 / HappyHorse 欢乐马）。

分镜大师与 KIE 的通讯架构：
- 自有服务器(/jy/api/v1/auth/api-key) → 返回 KIE API Key
- 分镜大师拿到 KIE Key 后直连 KIE API（api.kie.ai），参考图、提示词、生成结果均在客户端与 KIE 之间直接传递
- 不在自有服务器上中转参考图、提示词或生成图像

### 12.4 用户数据保护 — 核心链路（最高优先级，不可变更）

软件已进入生产期，有真实付费用户。

**核心链路（生存根本）**：`Chat → 宫格生成节点 → 视频生成节点`。
- 打开任何一个节点的视频生成页，看到的**必须和生成时一模一样**
- Chat 对话、宫格图、视频、参数 — 每一条都必须对应到节点上可见
- **禁止任何改动破坏这条链路的显示完整性**。断了 = 产品信用归零

**不可违反**：
1. **禁止删除 `app_data_dir/users/{id}/` 下的任何文件**
2. **禁止修改 `configs[nodeId]` 读取和表单回填逻辑**
3. **禁止修改 `saveConfig` / `hydrateVideoGenStore` / `addToHistory` 持久化链路**
4. **操作数据文件前必须先备份到 `.bak`**
5. **数据结构变更必须向后兼容**（新增字段用 optional）
6. **测试只能用测试账号**

涉及文件：
- `src/features/videoGeneration/videoGenStore.ts`
- `src/features/videoGeneration/VideoGenDialog.tsx`
- `src-tauri/src/commands/chat.rs` — `persist_videogen_store`、`load_videogen_store`
- `src-tauri/src/sync/mod.rs` — `migrate_old_user_data`、`get_user_dir`

涉及文件：
- `src/features/videoGeneration/videoGenStore.ts` — `saveConfig`、`hydrateVideoGenStore`、`addToHistory`、`reset`
- `src-tauri/src/commands/chat.rs` — `persist_videogen_store`、`load_videogen_store`、`videogen_store_path`
- `src-tauri/src/sync/mod.rs` — `migrate_old_user_data`、`get_user_dir`

---

如与以上全局约束冲突，以全局约束优先；如与运行时安全冲突，以安全优先。
